...
const RotatableGroup = ({ x, y, width, height, rotation = 0, children }: { x: number; y: number; width: number; height: number; rotation?: Rotation; children: ReactNode }) => (
  <Group x={x + width / 2} y={y + height / 2} rotation={rotation} offsetX={width / 2} offsetY={height / 2}>
...
export const ToiletSymbol = ({ x, y, width, height, color, rotation = 0 }: { x: number; y: number; width: number; height: number; color: string; rotation?: Rotation }) => (
  <RotatableGroup x={x} y={y} width={width} height={height} rotation={rotation}>
    <Rect x={width * 0.18} y={height * 0.05} width={width * 0.64} height={height * 0.22} stroke={color} strokeWidth={2} cornerRadius={4} />
    <Ellipse x={width * 0.5} y={height * 0.58} radiusX={width * 0.32} radiusY={height * 0.28} stroke={color} strokeWidth={2.2} />
    <Text x={0} y={height * 0.38} width={width} align="center" text="WC" fontSize={Math.max(9, Math.min(width, height) * 0.22)} fontStyle="bold" fill={color} />
  </RotatableGroup>
);
...
export const SinkSymbol = ({ x, y, width, height, color, rotation = 0 }: { x: number; y: number; width: number; height: number; color: string; rotation?: Rotation }) => (
  <RotatableGroup x={x} y={y} width={width} height={height} rotation={rotation}>
    <Ellipse x={width * 0.5} y={height * 0.46} radiusX={width * 0.38} radiusY={height * 0.3} stroke={color} strokeWidth={2.2} />
    <Circle x={width * 0.5} y={height * 0.46} radius={Math.max(2, Math.min(width, height) * 0.06)} fill={color} />
    <Line points={[width * 0.5, height * 0.12, width * 0.5, height * 0.3]} stroke={color} strokeWidth={2} />
    <Text x={0} y={height * 0.72} width={width} align="center" text="LV" fontSize={Math.max(8, Math.min(width, height) * 0.2)} fontStyle="bold" fill={color} />
  </RotatableGroup>
);
...
export const ShowerTraySymbol = ({ x, y, width, height, color, rotation = 0 }: { x: number; y: number; width: number; height: number; color: string; rotation?: Rotation }) => (
  <RotatableGroup x={x} y={y} width={width} height={height} rotation={rotation}>
    <Rect x={0} y={0} width={width} height={height} stroke={color} strokeWidth={2.4} cornerRadius={6} />
    <Line points={[width * 0.12, height * 0.2, width * 0.88, height * 0.8]} stroke={color} strokeWidth={1.3} dash={[4, 4]} />
    <Line points={[width * 0.88, height * 0.2, width * 0.12, height * 0.8]} stroke={color} strokeWidth={1.3} dash={[4, 4]} />
    <Circle x={width * 0.5} y={height * 0.5} radius={Math.max(3, Math.min(width, height) * 0.06)} stroke={color} strokeWidth={2} />
    <Text x={0} y={height * 0.08} width={width} align="center" text="DUCHA" fontSize={Math.max(8, Math.min(width, height) * 0.13)} fontStyle="bold" fill={color} />
  </RotatableGroup>
);
